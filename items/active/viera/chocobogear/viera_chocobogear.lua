...
function init()
  ...
  self.unlock = config.getParameter("unlock")
  ...
  animator.setPartTag("item", "unlock", string.format("%s",self.unlock))
...
function update(dt, fireMode, shiftHeld)
  ...
  if storage.firing and animator.animationState("firing") == "off" then
    item.consume(1)
	if player then	  
      world.sendEntityMessage(player.id(), "chocoboUnlock", self.category, self.unlock)
    end
